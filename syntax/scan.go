...
func (sc *scanner) scanRenderLit(val *tokenValue, litStart bool) Token {
	...
	for {
		...
		default:
			...
			if state.currentIndent < state.currentIndent {
				sc.error(val.pos, "render literal has bad indentation")
			}
...
func (sc *scanner) scanNumber(val *tokenValue, c rune) Token {
	// https://github.com/google/pkgscript-go/blob/master/doc/spec.md#lexical-elements
	//
